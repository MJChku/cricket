...
int exe_cuda_event_record_1(api_record_t* record)
{
    ...
    ptr arg1 = arg->arg1;
    ptr arg2 = arg->arg2;
    LOGE(LOG_DEBUG, "Exe cudaEventRecord");
...
bool_t cuda_event_record_1_svc(ptr event, ptr stream, timestamp ts, timedint *result, struct svc_req *rqstp)
{
    ...
    RECORD_ARG(1, event);
    RECORD_ARG(2, stream);
    LOGE(LOG_DEBUG, "Record cudaEventRecord %lu", ts);
...
int exe_cuda_event_record_with_flags_1(api_record_t* record)
{
    ...
    ptr arg2 = arg->arg2;
    int arg3 = arg->arg3;
    LOGE(LOG_DEBUG, "Exe cudaEventRecordWithFlags");
...
bool_t cuda_event_record_with_flags_1_svc(ptr event, ptr stream, int flags, timestamp ts, timedint *result, struct svc_req *rqstp)
{
    ...
    RECORD_ARG(2, stream);
    RECORD_ARG(3, flags);
    LOGE(LOG_DEBUG, "Record cudaEventRecordWithFlags %lu", ts);
...
bool_t cuda_event_synchronize_1_svc(ptr event, timestamp ts, timedint *result, struct svc_req *rqstp)
{
    NEX_RECORD_API(ptr);
    RECORD_SINGLE_ARG(event);
    LOGE(LOG_DEBUG, "Record cudaEventSynchronize %lu", ts);
...
int exe_cuda_launch_kernel_1(api_record_t* record)
{
    ...
    LOGE(LOG_DEBUG, "Exe cudaLaunchKernel(func=%p, gridDim=[%d,%d,%d], blockDim=[%d,%d,%d], args=%p, sharedMem=%d, stream=%p)",
                                ...
                                timedint *result, struct svc_req *rqstp)
{
    ...
    RECORD_ARG(5, sharedMem);
    RECORD_ARG(6, stream);
    LOGE(LOG_DEBUG, "Record cudaLaunchKernel %lu", ts);
    // fake return
    result->ret = 0;
    result->ts = ts;
